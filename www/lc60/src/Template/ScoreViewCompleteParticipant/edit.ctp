...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $scoreViewCompleteParticipant->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $scoreViewCompleteParticipant->id)]
            )
        ?></li>
...
<div class="scoreViewCompleteParticipant form large-9 medium-8 columns content">
    <?= $this->Form->create($scoreViewCompleteParticipant) ?>
    <fieldset>
        <legend><?= __('Edit Score View Complete Participant') ?></legend>
        <?php
